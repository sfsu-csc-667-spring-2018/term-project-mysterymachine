...
const draw_card = (game_id,num_of_cards,hand_id)=>
  db.one('WITH getval(card_id) AS (DELETE FROM active_pile WHERE ctid IN (SELECT ctid FROM active_pile WHERE game_id = ${1} ORDER BY random() LIMIT ${2}) RETURNING card_id) INSERT INTO hand_has_cards (hand_id,card_id) SELECT ${3}, card_id FROM getval;',[game_id,num_of_cards,hand_id]);
...
const new_game = (user_id) =>
  db.none(`WITH getval(game_id) as (INSERT INTO games (game_status, turn_order, active_seat,host_id) VALUES ('joining',1,1,${1}) RETURNING game_id) INSERT INTO game_has_hands (user_id, game_id, seat_number) SELECT ${1}, game_id, 1 FROM getval`,[user_id]);
